...
void correct_reed_solomon_decoder_create(correct_reed_solomon *rs) {
    ...
    rs->syndromes = calloc_ot(rs->min_distance, sizeof(field_element_t));
    rs->modified_syndromes = calloc_ot(2 * rs->min_distance, sizeof(field_element_t));
    ...
    rs->error_roots = calloc_ot(2 * rs->min_distance, sizeof(field_element_t));
    rs->error_vals = malloc_ot(rs->min_distance * sizeof(field_element_t));
    rs->error_locations = malloc_ot(rs->min_distance * sizeof(field_logarithm_t));
    ...
    rs->generator_root_exp = malloc_ot(rs->min_distance * sizeof(field_logarithm_t *));
    for (unsigned int i = 0; i < rs->min_distance; i++) {
        rs->generator_root_exp[i] = malloc_ot(rs->block_length * sizeof(field_logarithm_t));
    ...
    rs->element_exp = malloc_ot(256 * sizeof(field_logarithm_t *));
    for (field_operation_t i = 0; i < 256; i++) {
        rs->element_exp[i] = malloc_ot(rs->min_distance * sizeof(field_logarithm_t));
...
ssize_t correct_reed_solomon_decode_with_erasures(correct_reed_solomon *rs, const uint8_t *encoded,
    ...
    field_element_t *syndrome_copy = malloc_ot(rs->min_distance * sizeof(field_element_t));
